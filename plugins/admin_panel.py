...
def _admin_panel_kb():
    """Alias for admin_panel_inline to avoid ImportError in handlers/admin.py"""
...
def setup(app: Client) -> None:
    ...
    async def admin_fsm_handler(client: Client, message: Message):
        ...
        _admin_states = {
            ...
            States.ADMIN_ADD_DISCUSSION, # ← নতুন
            States.ADMIN_SET_GROUP,    # ← নতুন
            States.ADMIN_SET_DISCUSSION, # ← নতুন
        ...
        # ── Step 11: Discussion Link (Add Course flow) ──
        elif state == States.ADMIN_ADD_DISCUSSION:
            link = message.text.strip() if message.text else None
            data = get_data(uid)

            # If link is provided, use it. Otherwise, the default in db.add_course will be used.
            # But here we are persisting the course NOW.
            if link:
                data["discussion_group_link"] = link

            course_id = await _persist_course(data)
            clear_state(uid)

            await message.reply_text(
                f"✅ **Course & Discussion Link Saved!**\n\n"
                f"🆔 Course ID: `{course_id}`\n"
                f"🎓 Name: {data.get('name')}\n"
                f"🔗 Discussion: `{link if link else 'Default Link'}`\n",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=admin_panel_inline(),
            )

        # ── Set Discussion Link for Existing Course ──
        elif state == States.ADMIN_SET_DISCUSSION:
            link = message.text.strip() if message.text else None
            data = get_data(uid)
            course_id = data.get("course_id")

            if link:
                await db.set_course_group(course_id, 0, discussion_link=link) # Use 0 as placeholder for group_id since we only update link

            clear_state(uid)
            await message.reply_text(
                f"✅ **Discussion Link Updated!**\n\n"
                f"📦 Course: `{data.get('course_name')}`\n"
                f"🔗 Link: `{link if link else 'No change'}`\n",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=admin_back_panel_inline(),
            )
        ...
        # ── Set Group for Existing Course ── ← নতুন
            ...
            # Move to Discussion Link step
            set_state(uid, States.ADMIN_SET_DISCUSSION, get_data(uid))
            await checking.edit_text(
                f"✅ **Group Set!**\n\n"
                f"➕ **Discussion Group Link** দিন:\n\n"
                f"📌 Example: `https://t.me/+...`\n\n"
                f"💡 Skip করতে চাইলে `/cancel` দিন অথবা ডিফল্ট লিংক সেভ হবে।",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=admin_cancel_inline(),
            )
            clear_state(uid) # Temporary, will be handled by set_state above
            # Wait, I should not clear state before set_state. Fixed below.
