...
async def add_course(data: Dict[str, Any]) -> str:
    ...
    data.setdefault("discussion_group_link", "https://t.me/+WU_Qyp-jlNdjOWU1")
...
async def set_course_group(
    ...
    try:
        ...
        if discussion_link:
            update_data["discussion_group_link"] = discussion_link
...
async def is_banned(user_id: int) -> bool:
    db = get_db()
    user = await db.banned_users.find_one({"user_id": user_id})
